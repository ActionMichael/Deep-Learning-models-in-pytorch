# -*- coding: utf-8 -*-
"""
Created on Sat Jan 18 07:07:06 2020
...
#正式準備開始寫模型後遇到的第一個問題就是如何表示short connection，
#一個最直觀的想法是，手動編寫每一層，然後注意保存輸入x，在適當的時候把它進行處理，然後加到輸出裡。
#不過遇到以下問題
#1.如果要對網絡進行修改，會比較麻煩。
#2.18層的resnet這樣寫是可行的，但是論文中提到了有建造1000層的resnet，如果這樣寫的話，1000層寫起來就太麻煩了。
#考量上述這裡將重複性質高的打包成block的概念不斷去調用組建起來
...
#定義BasicBlock(打包，適用於18、34層的resnet){又有別稱residual block(殘差模塊)}
class BasicBlock(nn.Module):
    #先看看左邊的BasicBlock類。先是定義了一個conv3*3的函數卷積操作留待後面用，
    #在BasicBlock類裏兩個3*3的卷積爲主要部分，用了batchnorm和relu激活函數，
    #其中downsample在後面_make_layer函數會有定義，它其實就是一個普通的卷積過程（卷積核大小爲1），字面意思就是下采樣
    #與基礎版的不同之處只在於這裡是三個卷積，分別是1x1,3x3,1x1,(在rasnet50以上有用到Bottleneck，該expansion=4)
    #分別用來壓縮維度，卷積處理，恢復維度，inplanes是輸入的通道數，
    #planes是輸出的通道數，expansion是對輸出通道數的倍乘，
    #在basic中expansion是1，此時完全忽略expansion這個東東，輸出的通道數就是planes，
    #然而bottleneck就是不走尋常路，它的任務就是要對通道數進行壓縮，再放大，
    #於是，planes不再代表輸出的通道數，而是block內部壓縮後的通道數，
    #輸出通道數變為planes*expansion。 接著就是網絡主體了。
    expansion = 1
    #inplanes其實就是channel，叫法不同；而planes則是因為這裡expension=1所以剛好代表輸出通道
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        self.conv1 = conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        #inplace=True是否將計算得到的值直接覆蓋之前的值，節省顯卡內存空間
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = nn.BatchNorm2d(planes)
        #expansion是跟downsample(降採樣)有關的，他的直面意思是擴張，
        #因爲殘差層裏輸入輸出需要保持一致，所以不一致就要用到downsample(降採樣)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, grayscale):
        ...
        #此時outplanes=128而self.inplanes=64為上basic_block的輸出channel，
        #此時channel不一致，需要對輸出的x擴維後才能相加，downsample實現的就是該功能
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)
        #此時outplanes=256而self.inplanes=128為，此時也需要擴維後才能相加，downsample實現的就是該功能
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2)
        #此時outplanes=512而self.inplanes=256為，此時也需要擴維後才能相加，downsample實現的就是該功能
    ...
    #https://towardsdatascience.com/deep-dive-into-the-computer-vision-world-f35cd7349e16
    #http://www2.kaiyodai.ac.jp/~takenawa/learning/10_1ResNet_GPU.html
